...
import static com.grame.services.fees.bootstrap.JsonToProtoSerde.loadFeeScheduleFromJson;
...
public class HfsSystemFilesManager implements SystemFilesManager {
	...
	private final TieredgrameFs hfs;
	...
	private byte[] bioAndIpv4Contents() {
		var basics = com.gramegrame.api.proto.java.AddressBook.newBuilder();
	...
	private byte[] bioAndPubKeyContents() {
		var details = com.gramegrame.api.proto.java.AddressBook.newBuilder();
		LongStream.range(0, currentBook.getSize())
				.mapToObj(currentBook::getAddress)
				.map(address ->
						basicBioEntryFrom(address)
