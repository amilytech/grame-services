...
import static com.grame.services.context.properties.BootstrapProperties.GLOBAL_DYNAMIC_PROPS;
import static com.grame.services.context.properties.BootstrapProperties.transformFor;
...
public class ScreenedSysFileProps implements PropertySource {
	...
	private static Map<String, String> STANDARDIZED_NAMES = Map.ofEntries(
			...
			entry("txMaximumDuration", "grame.transaction.maxValidDuration"),
			entry("txMinimumDuration", "grame.transaction.minValidDuration"),
			entry("txMinimumRemaining", "grame.transaction.minValidityBufferSecs"),
	...
	private static Map<String, Predicate<Object>> VALUE_SCREENS = Map.ofEntries(
			...
			entry("scheduling.whitelist",
					whitelist -> ((Set<grameFunctionality>)whitelist)
							.stream()
