...
import static com.grame.services.context.properties.BootstrapProperties.NODE_PROPS;
...
public class ScreenedNodeFileProps implements PropertySource {
	...
	static Map<String, String> STANDARDIZED_NAMES = Map.ofEntries(
			...
			entry("recordStreamQueueCapacity", "grame.recordStream.queueCapacity"),
			entry("enableRecordStreaming", "grame.recordStream.isEnabled"),
			entry("recordLogDir", "grame.recordStream.logDir"),
			entry("recordLogPeriod", "grame.recordStream.logPeriod"),
			entry("tlsPort", "grpc.tlsPort"),
			entry("environment", "grame.profiles.active"),
